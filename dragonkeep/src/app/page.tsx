...
import { formatNumber, pendingCoins, pendingFood } from "@/game/economy";
...
export default function Page() {
  ...
  const foodWaiting = save.bakeries.reduce((n, b) => n + pendingFood(pack, b, now), 0);
      ...
      <header className="sticky top-0 z-20 border-b border-line bg-ink/95 backdrop-blur">
        ...
        <div className="flex items-center gap-4 px-4 py-2.5">
          <div>
            ...
            <p className="num text-base leading-tight">{formatNumber(save.coins)}</p>
          </div>
          <div>
            <p className="eyebrow">Food</p>
            <p className="num text-base leading-tight">
              {formatNumber(save.food)}
              {foodWaiting > 0 && (
