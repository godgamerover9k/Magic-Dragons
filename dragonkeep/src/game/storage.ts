import { defaultContentPack, SCHEMA_VERSION } from "./content";
...
export function loadPack(): ContentPack {
  ...
  try {
    ...
    return migratePack(JSON.parse(raw) as ContentPack);
  } catch {
 ...
 */
export function migrateSave(_pack: ContentPack, save: SaveGame): SaveGame {
  const dragons = save.dragons.map((dragon) => {
    ...
    delete patched.traits;
    delete patched.ivs;
    return patched as Dragon;
  });
  return { ...save, schemaVersion: SCHEMA_VERSION, dragons };
}
