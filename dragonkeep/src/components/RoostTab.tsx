...
function DragonCard({
  ...
  const affordableFood = pack.balance.foodTypes.filter(
    (f) => f.unlocksAtBakeryLevel <= Math.max(1, ...save.bakeries.map((b) => b.level)),
  );
      ...
      {open && (
        <div className="space-y-3 border-t border-line p-3">
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
            ...
            <Stat label="Generation" value={String(dragon.generation)} />
            <Stat label="Origin" value={dragon.source} />
            <Stat label="Merges absorbed" value={String(dragon.mergeCount)} />
          </div>
